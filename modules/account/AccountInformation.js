...
import PlannerProjects from "./PlannerProjects";
...
class AccountInformation extends Component {
    ...
    saveAccountData(e,type){

        console.log('eeeeeeeeee', e);

        e.preventDefault();
        return store.dispatch(formSubmit(e,false,false)).then(res=>{
            if(res.data.errors){
                        ...
                        <div className={'account-information-wrapper'}>
                            <div className="account-sidebar">
                                <div className={'account-sidebar-title'}>
                                    <div className="account-name"><b>Hello, {this.props.customer.name}!</b><p>{this.props.customer.email}</p></div>
                                    <div className={'logout-btn'} onClick={e=>customerLogout(e, this.props.history)}>Log Out</div>
                                </div>
                                <AccountSidebar account={account} history={this.props.history}/>
                            </div>
                            <div className="account-content">
                                ...
                                <AccountTabs
                                    ...
                                    saveAccountData={this.saveAccountData.bind(this)}
                                    setResponses={this.setResponses.bind(this)}
                                    history={this.props.history}
                                />
                                {this.state.errors? <FormErrors onCloseModal={this.onCloseErrorsModal.bind(this)} formErrors={this.state.errors} />:''}

                            </div>
                        </div>
                    </section>
                    {this.props.location && this.props.location.state && this.props.location.state.message ?
                        <Modal open={this.state.activateModal} onClose={e=>this.setState({activateModal:false})} blockScroll={false} center
                               styles={{
                                   modal: {
                                       maxWidth: "350px",
                                       textAlign: 'center',
                        ...
                        >
                            <div className="modal-body">
                                <div className={'check-email-title'}>{this.props.location.state.message}</div>
                                <p className={'check-email-text'}>Now you can see the prices and place your first order.</p>
                            </div>
                        </Modal>
                        : ''
                    }
...
const AccountSidebar=({account,history})=>{
    ...
    return <ul className="account-navigation">
        <li className={"navigation-item"+((history.location.hash=='') ? ' active'  : '')} >
            <Link   to={'/account-information'}>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 350 350" style={{enableBackground: "new 0 0 350 350"}} xmlSpace="preserve"><g><path d="M175,171.2c38.9,0,70.5-38.3,70.5-85.6C245.5,38.3,235.1,0,175,0s-70.5,38.3-70.5,85.6C104.5,132.9,136.1,171.2,175,171.2z"/><path d="M41.9,301.9C41.9,299,41.9,301,41.9,301.9L41.9,301.9z"/><path d="M308.1,304.1C308.1,303.3,308.1,298.6,308.1,304.1L308.1,304.1z"/><path d="M307.9,298.4c-1.3-82.3-12.1-105.8-94.4-120.7c0,0-11.6,14.8-38.6,14.8s-38.6-14.8-38.6-14.8c-81.4,14.7-92.8,37.8-94.3,118c-0.1,6.5-0.2,6.9-0.2,6.1c0,1.4,0,4.1,0,8.7c0,0,19.6,39.5,133.1,39.5c113.5,0,133.1-39.5,133.1-39.5c0-3,0-5,0-6.4C308.1,304.6,308,303.7,307.9,298.4z"/></g></svg>
                Account Information</Link>
        </li>
        {!!isPartner&&
        <li className={"navigation-item"+((history.location.hash=='#partner-orders') ? ' active'  : '')} >
            <Link  to={'/account-information/#partner-orders'}>
        ...
        {!!isCustomer&&
        <li className={"navigation-item"+((history.location.hash=='#orders') ? ' active'  : '')} >
            <Link  to={'/account-information/#orders'}>
                <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 512 512" style={{enableBackground: "new 0 0 512 512"}} xmlSpace="preserve"><g><path d="M75,497c0,8.3,6.7,15,15,15h331c8.3,0,15-6.7,15-15v-45H75V497z"/><path d="M421,86H90c-8.3,0-15,6.7-15,15v321h361V101C436,92.7,429.3,86,421,86z M197.1,352.1l-29.5,29.5c-2.8,2.8-6.6,4.4-10.6,4.4s-7.8-1.6-10.6-4.4l-12-12c-5.9-5.9-5.9-15.4,0-21.2c5.9-5.9,15.4-5.9,21.2,0l1.4,1.4l18.9-18.9c5.9-5.9,15.4-5.9,21.2,0C203,336.8,203,346.2,197.1,352.1z M197.1,263.1l-29.5,29.5c-2.8,2.8-6.6,4.4-10.6,4.4s-7.8-1.6-10.6-4.4l-12-12c-5.9-5.9-5.9-15.4,0-21.2c5.9-5.9,15.4-5.9,21.2,0l1.4,1.4l18.9-18.9c5.9-5.9,15.4-5.9,21.2,0C203,247.8,203,257.2,197.1,263.1z M197.1,173.6l-29.5,29.5c-2.9,2.9-6.8,4.4-10.6,4.4s-7.7-1.5-10.6-4.4l-12-12c-5.9-5.9-5.9-15.4,0-21.2c5.9-5.9,15.4-5.9,21.2,0l1.4,1.4l18.9-18.9c5.9-5.9,15.4-5.9,21.2,0C203,158.3,203,167.8,197.1,173.6z M361,372H246.2c-8.3,0-15-6.7-15-15s6.7-15,15-15H361c8.3,0,15,6.7,15,15S369.3,372,361,372z M361,283H246.2c-8.3,0-15-6.7-15-15s6.7-15,15-15H361c8.3,0,15,6.7,15,15S369.3,283,361,283z M361,193.5H246.2c-8.3,0-15-6.7-15-15c0-8.3,6.7-15,15-15H361c8.3,0,15,6.7,15,15S369.3,193.5,361,193.5z"/><path d="M205,15c0-8.3-6.7-15-15-15h-70c-8.3,0-15,6.7-15,15v41h100V15z"/><path d="M406,15c0-8.3-6.7-15-15-15h-70c-8.3,0-15,6.7-15,15v41h100V15z"/></g></svg>
                Order History</Link></li>
        }
        {!!isCustomer&&
        <li className={"navigation-item"+((history.location.hash=='#saved-cart') ? ' active'  : '')} >
            <Link  to={'/account-information/#saved-cart'}>
                <i className='fa fa-save fa-2x'/>
                Saved Projects</Link></li>
        }
{/*        {!!isCustomer&&
        <li className={"navigation-item"+((history.location.hash=='#planner-projects') ? ' active'  : '')} >
            <Link  to={'/account-information/#planner-projects'}>
                ...
                3D Planner Projects</Link></li>
        }*/}
        {/*<li className={"navigation-item"+((history.location.hash=='#referrals') ? ' active'  : '')} >*/}
        {/*    <Link  to={'/account-information/#referrals'}>*/}
        {/*        <i className='fa fa-save fa-2x'/>*/}
        {/*        Referrals*/}
        {/*    </Link>*/}
        {/*</li>*/}
        {(account && account.customer_roles && account.customer_roles.designer ) &&
            <>
                <li className={"navigation-item"+((history.location.hash=='#my-projects') ? ' active'  : '')} >
                    <Link  to={'/account-information/#my-projects'}>
                        My Projects</Link>
                </li>
                <li className={"navigation-item"+((history.location.hash=='#my-projects') ? ' active'  : '')} >
                    <SignToFee4beeDesigner styled={true}/>
                </li>
...
export class AccountTabs extends React.Component {
    ...
    render() {
        ...
        let isPartner = this.props.account && this.props.account.customer_roles && this.props.account.customer_roles['partner'];
        let isCustomer = this.props.account && this.props.account.customer_roles && this.props.account.customer_roles['customer'];


        switch (this.props.history.location.hash){
            case '#orders':
                return !!isCustomer &&  <AccountOrders
                    edit = {this.props.edit}
                    shipping = {this.props.shipping}
                    account = {this.props.account}
                    customer = {this.props.customer}
                    changeEdit={this.props.changeEdit.bind(this)}
                    changeShipping={this.props.changeShipping.bind(this)}
                    saveAccountData={this.props.saveAccountData.bind(this)}
                    url={'/orders/customer?status=pending,completed,in_process,canceled,partial_paid&customer_hide=1'}
                    setResponses={this.props.setResponses.bind(this)}

                />;
            case '#partner-orders':
                return !!isPartner &&  <AccountOrders
                    edit = {this.props.edit}
                    shipping = {this.props.shipping}
                    account = {this.props.account}
                    customer = {this.props.customer}
            ...
            case '#saved-cart':
                return !!isCustomer &&  <AccountOrders
                    ...
                    saveAccountData={this.props.saveAccountData.bind(this)}
                    savedCart={true}
                    url={'/orders/customer?status=saved_cart'}
                    setResponses={this.props.setResponses.bind(this)}
                />;
            case '#my-projects':
            ...
            case '#planner-projects':
                return !!isCustomer &&  <PlannerProjects
                    account = {this.props.account}
                    customer = {this.props.customer}
                    setResponses={this.props.setResponses.bind(this)}
                />;
            case '#referrals':
                return !!isCustomer &&  <Referrals
                    account = {this.props.account}
                    customer = {this.props.customer}
                    setResponses={this.props.setResponses.bind(this)}
                />;
...
export default connect(mapStateToProps)(withRouter(AccountInformation));
